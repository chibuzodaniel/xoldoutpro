...
import { getAmbassadorRevenueGeneratedKobo, getAmbassadorTierRatePercent, getWalletBalances } from "@/lib/commerce/ledger";
  ...
  try {
    ...
    const ambassadorRows = await Promise.all(
      ambassadors.map(async (a) => {
        const [referredCount, revenueGeneratedKobo, { availableKobo }] = await Promise.all([
