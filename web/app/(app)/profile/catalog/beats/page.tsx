...
function ProductEditor({ product, onSaved }: { product: CatalogProduct; onSaved: () => void }) {
  ...
  async function handleSave() {
    ...
    setBusy(true);
    try {
      const res = await apiFetch(`/api/beats/${product.id}`, { method: "PATCH", body: JSON.stringify(body) });
      if (!res.ok) {
              ...
              <div key={p.id} className="py-3">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded bg-surface-2 shrink-0 overflow-hidden">
                    <FallbackImg src={cover} alt={p.title} className="h-full w-full object-cover" fallback={null} />
                  </div>
